...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
		...
		if (parent.down[0] != null && parent.down[0].equals(child)) {
			...
			// not the root, move child up
			child.up = parent.up;
				// check which side the child should be connected to
				if (parent.up.down[0] != null && parent.up.down[0].equals(parent)) {
				...
				// root, child will become new root
				root = child;
				child.up = null;
			}
	    	// point nodes to correct places
	    	parent.up = child;
		...
		// not related, throw error
		else {
	    	throw new IllegalArgumentException("No parent and child relationship between nodes");
		}
    }
    ...
    /**
     * Tests the correctness of the rotate method using invalid inputs (nodes provided are not 
     * connected directly as child and parent)
     * 
	...
	boolean test1 = false;
	try {
	    test.rotate(test.findNode(B), test.findNode(E));
	...
	boolean test2 = false;
	try {
	    test.rotate(test.findNode(A), test.findNode(C));
	} catch(IllegalArgumentException e) {
	    test2 = true;
	}
	...
	boolean test3 = false;
	try {
	    test.rotate(test.findNode(C), test.findNode(B));
	} catch(IllegalArgumentException e) {
	    test3 = true;
	}
	...
	boolean test4 = false;
	try {
	    test.rotate(test.findNode(E), test.findNode(C));
	} catch(IllegalArgumentException e) {
	    test4 = true;
	}
	...
	// rotate node B and node D
	test.rotate(test.findNode(B), test.findNode(D));
	// check if the tree is still valid using in order traversal
	String expectedIn = "[ A, B, C, D, E ]";
	if (!expectedIn.equals(test.toInOrderString())) {
	    return false;
	}
	// check results with level order traversal
	String expectedLevel = "[ B, A, D, C, E ]";
	if (!expectedLevel.equals(test.toLevelOrderString())) {
	    return false;
	}
	
	// Case 2: rotation not at the root
	// set up tree
	test.clear();
	test.insert(F);
	test.insert(G);
	...
	test.rotate(test.findNode(B), test.findNode(D));
	// check if the tree is still valid using in order traversal
	expectedIn = "[ A, B, C, D, E, F, G ]";
	if (!expectedIn.equals(test.toInOrderString())) {
	    return false;
	}
	// check results with level order traversal
	expectedLevel = "[ F, B, G, A, D, C, E ]";
	if (!expectedLevel.equals(test.toLevelOrderString())) {
	    return false;
	}

	// Case 3: rotation not at the root, but parent is to the right of the root
	// set up tree
	test.clear();
	test.insert(B);
	test.insert(A);
	...
	test.rotate(test.findNode(D), test.findNode(F));
	// check if the tree is still valid using in order traversal
	expectedIn = "[ A, B, C, D, E, F, G ]";
	if (!expectedIn.equals(test.toInOrderString())) {
	    return false;
	...
	// rotate node B and node D
	test.rotate(test.findNode(D), test.findNode(B));
	// check if the tree is still valid using in order traversal
	String expectedIn = "[ A, B, C, D, E ]";
	if (!expectedIn.equals(test.toInOrderString())) {
	    return false;
	...
	// check results with level order traversal
	String expectedLevel = "[ D, B, E, A, C ]";
	if (!expectedLevel.equals(test.toLevelOrderString())) {
	    return false;
	}
	...
	test.rotate(test.findNode(F), test.findNode(D));
	// check if the tree is still valid using in order traversal
	expectedIn = "[ A, B, C, D, E, F, G ]";
	if (!expectedIn.equals(test.toInOrderString())) {
	    return false;
	...
	// check results with level order traversal
	expectedLevel = "[ B, A, F, D, G, C, E ]";
	if (!expectedLevel.equals(test.toLevelOrderString())) {
	    return false;
	}
	
	// Case 3: rotation not at the root, but parent is to the left of the root
	// set up tree
	test.clear();
	test.insert(F);
	test.insert(G);
