...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
      ...
      if (child == null || parent == null || child.up != parent || (parent.down[0] != child && parent.down[1] != child)) { // check for child/parent
        throw new IllegalArgumentException();
      }
      if (parent.equals(root)) { //always move child up to parent's spot
        root = child;
        child.up = null;
      } else {
      ...
      parent.up = child;
      if (parent.down[0] == child) { // if left child, right rotation
        parent.down[0] = child.down[1];
        if (child.down[1] != null) {
            child.down[1].up = parent;
        }
        child.down[1] = parent;
      } else { // if right child, left rotation
    ...
    /**
     * Tests if class correctly throws exception when new nodes are unrelated.
    ...
    public static boolean test1() {
      ...
      Node<Integer> leftChild = new Node<Integer>(2);
      Node<Integer> leftGrandchild = new Node<Integer>(1);
      tree.insert(3);
      tree.insert(2);
      tree.insert(1);
      boolean errorTest = false;
      try {
        tree.rotate(leftGrandchild, root);
    ...
    /**
     * Tests right rotation from root
     * @return true if test passes, else false
     */
    public static boolean test2() {
        ...
        if (!tree.root.equals(tree.findNode(2))) {
          return false;
        }
        if (tree.toLevelOrderString() != "[ 2, 1, 3 ]") {
    ...
    /**
     * Tests left rotation from child of root
     * @return true if test passes, else false
     */
    public static boolean test3() {
        ...
        tree.insert(5);
        tree.insert(7);
        tree.rotate(tree.root.down[1].down[1], tree.root.down[1]);
        if (!tree.root.equals(tree.findNode(2))) {
          return false;
        }
        if (tree.toLevelOrderString() != "[ 2, 1, 6, 4, 7, 3, 5 ]") {
          System.out.println(tree.toLevelOrderString());
          return false;
        }
        return true;
